...
export async function getAIResponse(messages) {
  ...
  if (groq.keys.length) {
    const result = await callOpenAICompatible(groq, messages);
    if (result) return result;
    // Groq's rate-limit window won't clear in 3 seconds, so a blind
    // retry-then-wait here just adds guaranteed dead time to every request
    // that hits it — fall straight through to the next provider instead.
    console.log("All Groq keys failed on this pass — falling through to Gemini");
  }
...
export async function transcribeAudio(buffer, filename, mimetype) {
  ...
  for (const key of keys) {
    try {
      ...
      // Force English. Without this, Whisper auto-detects the spoken language
      // per request, and any noisy/unclear audio can get misdetected as a
      // different language entirely — garbling the transcript and then
      // causing the downstream chat model to reply in that wrong language too.
      form.append("language", process.env.TRANSCRIBE_LANGUAGE || "en");
